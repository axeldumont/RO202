    ...
    def branch(self, tree):
        ...
        I - Description des variables et de leurs attributs que vous devrez utiliser
         ...
         - Variable tree : représente l'arbre de branchement dans lequel se trouve le sommet. Ses attributs sont :
           ...
           node.branch(tree)
        """
        if self.tableau.bestSolution is not None:
            # J'ai ici changé bbTree pour que tree.bestSolution soit fixé à None et non np.array([])
            if tree.bestSolution is None or (self.tableau.isMinimization and tree.bestObjective > self.tableau.bestObjective) or (not self.tableau.isMinimization and tree.bestObjective < self.tableau.bestObjective):
                id = -1
                for i in range(self.tableau.bestSolution.shape[0]):
                    if isFractional(self.tableau.bestSolution[i]):
                        id = i
                        if self.depth == 0:
                            print(f'root : x[{id+1}] = {self.tableau.bestSolution[i]}')
                        break

                if id == -1: 
                    tree.bestObjective = self.tableau.bestObjective
                    tree.bestSolution = self.tableau.bestSolution
                    print(self.depth * " "+"Integer Solution :")
                    self.tableau.displaySolution()
                
                else:
                    newA = np.array([0.0] * self.tableau.n)
                    newA[id] = 1
                    newRhs = math.floor(self.tableau.bestSolution[id])

                    branch_low = BBNode.create_non_root(self, newA, newRhs)
                    print((self.depth+1) * " " + f"x[{id+1}] <= {newRhs} : ")
                    branch_low.branch(tree)

                    newA = np.array([0.0] * self.tableau.n)
                    newA[id] = - 1
                    newRhs = - math.ceil(self.tableau.bestSolution[id])

                    branch_up = BBNode.create_non_root(self, newA, newRhs)
                    print((self.depth+1) * " " + f"x[{id+1}] >= {newRhs} : ")
                    branch_up.branch(tree)
            else:
                print("Node cut : relaxation worst than bound"+self.depth * " " + f"(node relaxation : {self.tableau.bestObjective}, best integer solution : {tree.bestObjective})")
        else:
            print("Node cut : infeasable solution")
    ...
    def __init__(self, A=None, rhs=None, obj=None, isMinimization=None, parent=None, newA=None, newRhs=None):
        ...
        if parent == None:
            ...
            self.tableau = tableau.Tableau(
                newMA, newMRhs, parent.tableau.c, parent.tableau.isMinimization)
