...
########### MISE SOUS FORME CANONIQUE ############
        ...
        for i in range(self.m):
            ...
            # Division par A[i, basis[i]] pour fixer le coefficient A[i, basis[i]] à 1
            pivot = self.A[i][self.basis[i]] 
            print(pivot)        
            for j in range(self.n):
                self.A[i][j] /= pivot           # On divise toute la ligne par le pivot
            self.b[i] /= pivot                  # On met à jour b
            
            # Combinaison linéaire pour fixer les autres coefficients de la colonne basis[i] à 0
            for j in range(self.m):
                if j != i:                              # Pour tous les autres coefs de la colonne basis[i]
                    ratio = self.A[j][self.basis[i]]              # Le coef multiplicateur est le coef de la colonne
                     
                    for k in range(self.n):                  # Pour toute la ligne j               
                        self.A[j][k] -= ratio * self.A[i][k]      # On modifie linéairement
                    
                    self.b[j] -= ratio * self.b[i]                # On met à jour b
        
            '''
        Cet algorithme mérite une explication :
           - On met le pivot à 1 (très simple)
           - On pose ratio = l'élément de la colonne basis[i] que l'on veut rendre nul
           - Pour tous les coefficients sur la meme ligne que cet élément:
                - On leur soustrait ratio * le coef de la ligne i et de la même colonne 
                - En gros, on fait (Cj) = (Cj) - ratio * (Ci) !

            - En répétant cette opération pour chaque élément de basis[i] non nul, pour tous les i, on aura bien le tableau espéré
            '''
            
            # combinaison linéaire pour fixer c[basis[i]] à 0
            objective = self.c[self.basis[i]]             # On prend c[basis[i]] que l'on veut rendre nul
            if objective !=0.0:
                for j in range(self.n):                 
                    self.c[j] -= objective * self.A[i][j]    # On fait (Obj) = (Obj) - objective * (Ci) 
            
                self.bestObjective -= objective * self.b[i] # On met à jour bestObjective. Le meilleur objectif est la valeur absolue de BestObjective.
        
        ################## FIN DE LA MISE SOUS FORME CANONIQUE #############3####
        ...
        """
        max_temp = sys.float_info.max
        sortant = sys.maxsize
        for i in range(len(self.basis)):
            if self.A[i][entrant] > 1e-6:
                ratio = self.b[i] / self.A[i][entrant]
                if ratio > 1e-6 and ratio < max_temp:
                    sortant = i
                    max_temp = ratio
            else:
                ratio = sys.float_info.max
        if sortant != sys.maxsize:
            print(f"Variable sortante : x{self.basis[sortant]+1}\n")
            print(f"Ancienne base : {self.basis}")
            self.basis[sortant] = entrant
            print(f"Nouvelle base : {self.basis}")
            return True
    ...
    def setSolution(self, tSlack):
        ...
        for varId in range(self.n):
            ...
            print("varId = ", varId, " solution value: ", "%.2f" %
                  tSlack.bestSolution[varId])
...
def ex2():
    ...
    A = np.array([[1, -2, 1, -1, 0, 0], [0, 1, 3, 0, 1, 0],
                 [2, 0, 1, 2, 0, 1]], dtype=float)
