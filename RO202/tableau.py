    ...
    def pivot(self):
        ...
        for i in range(self.m):
            ...
            for row in range(self.m):
                if row != i:
                    objective = self.A[row][col]
                    for k in range(self.n):
                        self.A[row][k] -= objective * self.A[i][k]
                    self.b[row] -= objective * self.b[i]

            if self.c[col] != 0:
                objective = self.c[col]
                for k in range(self.n):
                    self.c[k] -= objective * self.A[i][k]
                self.bestObjective -= objective * self.b[i]
